...
- **可观测、易部署、数据自持** — 提供健康、路由、日志、用量与成本估算；单个 Go 二进制内嵌管理界面，支持 SQLite、MySQL、PostgreSQL 和本地凭据加密。请求日志在处理中和完成后都统一显示请求开始时间。
...
新版和经典版共用这些策略。新版分组入口为「分组详情 → 高级配置 → 分组并发」，经典版位于分组设置；密钥和账号/凭据在各自编辑页设置。数据库自动应用 `0016_concurrency`，保留主干的用量索引；已运行原 `0015_concurrency` 版本的数据库也会保留已有策略并自动升级。

在「设置 → 并发限制」配置全局请求上限，以及分组、AccessKey、账号/凭据的默认上限；各分组设置、AccessKey 编辑页和凭据详情可单独覆盖。选择继承使用对应系统默认值，`0` 表示不限，正整数上限为 1,000,000。初始默认均不限；设置、分组和 AccessKey 的并发修改使用页面主保存按钮统一应用，新版凭据详情的并发、权重和代理也通过主保存按钮一次提交；经典版凭据保持独立编辑器。

AccessKey 按完整客户端请求计数，包括内部重试；分组和凭据按正在执行的上游尝试计数，SSE 持续占用至结束，WebSocket 按每轮请求计数。同一订阅账号通过稳定身份跨分组共享上限和计数；后台凭据探测也遵守上游限额。管理 API、模型列表和空闲 WebSocket 连接不占用请求名额，WebSocket 连接保护仍独立生效。

AccessKey 满载返回 `429 access_key_concurrency_limit`；全局满载返回 `503 global_concurrency_limit`。满载分组/账号会被跳过，候选容量全部耗尽返回 `503 upstream_concurrency_limit`，不排队、不消耗上游重试次数、不影响账号健康。已绑定身份的续接请求不会为了绕过限额切换账号。降低上限保留已有请求；内存计数在重启后归零，策略保留。

首页、分组、账号/凭据和 AccessKey 页面每约 3 秒更新「当前并发 / 上限」。首页区分客户端处理中请求和上游执行数；AccessKey 登录用户只能读取自己的计数。限制与统计仅作用于当前实例。
