...
- **可観測で導入しやすく、データを自己管理** — 健全性、ルート、ログ、使用量、コスト概算を確認でき、SQLite、MySQL、PostgreSQL とローカル認証情報暗号化を単一バイナリで利用できます。リクエストログの時刻は処理中と完了後で同じリクエスト開始時刻を表示します。
...
新版とクラシック版は同じポリシーを共有します。新版ではグループ詳細の詳細設定からグループ同時実行数を設定し、クラシック版ではグループ設定を使用します。キーとアカウント・認証情報は各編集画面で設定できます。`0016_concurrency` が自動適用され、主幹の使用量インデックスを維持します。旧 `0015_concurrency` 版のデータベースも既存ポリシーを保持して更新されます。

「設定 → 同時実行数の制限」で全体のリクエスト上限と、Group・AccessKey・アカウント/認証情報の既定上限を設定できます。各設定画面で個別に上書きできます。継承は対応するシステム既定値、`0` は無制限、正整数は最大 1,000,000 です。初期状態はすべて無制限です。設定・Group・AccessKey の変更は各画面のメイン保存ボタンでまとめて適用し、新版の認証情報詳細でも同時実行数・重み・プロキシをまとめて保存します。クラシック版の認証情報は個別の同時実行数エディターを維持します。

AccessKey は再試行を含むクライアントリクエスト全体、Group と認証情報は各上流リクエストの実行中を数えます。SSE は終了まで、WebSocket は各ターン単位で数えます。同じサブスクリプションアカウントは Group 間で上限と実行数を共有し、認証情報の検査にも上流制限を適用します。管理 API、ローカルのモデル一覧、アイドル状態の WebSocket は対象外で、接続数の保護は別に適用されます。

AccessKey の上限到達は `429 access_key_concurrency_limit`、全体は `503 global_concurrency_limit` を返します。満杯の Group/アカウントはスキップし、すべて満杯なら `503 upstream_concurrency_limit` を返します。待機キューはなく、再試行回数や健全性に影響しません。上限を下げても実行中のリクエストは中断せず、アカウントに固定された会話を別アカウントへ切り替えることもありません。

各画面の実行数は約 3 秒ごとに更新されます。AccessKey でログインした利用者は自分の値のみ参照できます。設定は保存され、実行数は再起動時にゼロになります。制限と統計は単一インスタンス内に適用されます。
