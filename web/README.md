...
- `src/frontends/classic/`：保留经典前端，原 `@/` 别名指向此目录。功能迁移在两套界面各自实现，避免无关的页面、交互和路径改写。
...
浏览器界面偏好使用 `gpt-load.frontend.v2`：选择新版时保存 `modern`，选择经典版时清除该值，默认 `classic`。旧键 `gpt-load.frontend` 在启动时清理，不再参与界面选择。新版顶栏的显示模式菜单与经典版顶栏菜单均提供「新版 / 经典版」切换，对所有人开放，包括登录页和访问密钥用户。顶栏切换后重新加载首页，登录页切换则留在登录页；设置页原有入口仍返回 `/settings`。每次只创建一个应用和路由实例。

启动选择器直接读取浏览器界面偏好，不查询认证接口。刷新、退出登录和认证失效均保留选择；存储读取不可用或偏好值无效时默认使用经典版。选择界面不改变身份、路由和接口权限，两套界面各自验证会话并限制管理员页面。语言和明暗模式保留既有浏览器存储约定。

旧版全局设置增加界面选择和示意缩略图。存在未保存修改或设置操作尚未结束时，需先完成原有保存/放弃流程。两套界面的顶栏切换复用当前页面的未保存修改确认；正在保存时不切换，确认放弃后不重复弹出浏览器离开提示。偏好无法保存时显示错误并保留当前界面和草稿。
...
两套界面均提供全局、分组、密钥、账号/凭据的并发配置与实时计数，各自持有 API 类型和组件，使用同一套服务端策略。新版入口为「设置 → 并发限制」「分组详情 → 高级配置 → 分组并发」、密钥编辑及凭据详情；经典版入口保持在对应设置页。分组和密钥列表单列展示并发，首页区分全局请求和上游执行数。计数按当前页面目标批量读取，约每 3 秒刷新，后台暂停轮询。

全局设置、分组设置和密钥编辑将并发字段并入原更新请求，在服务端事务内一并保存；失败保留草稿供重试。重置先还原为继承状态，点击主保存后生效。新版凭据详情使用「继承 / 不限 / 自定义」分段选项，并发、权重与代理通过底部主按钮一次保存；经典版凭据保持独立编辑器。同一订阅账号跨分组共享并发策略和计数，普通凭据按 ID 独立计数。访问密钥会话只查询自己的密钥并发，不请求管理员的全局、分组或账号计数。切换界面沿用未保存修改保护。

数据库迁移使用 `0016_concurrency`，保留主干的 `0015_group_usage_index`。曾运行原并发提交的数据库会在迁移锁内识别 `0015_concurrency`，保留策略表和数据后补齐主干索引并重新登记编号。
